#!/usr/bin/env python3
"""Evaluate analytical ENFF, AD-EnFF, and MNMEF on one Lorenz-63 protocol."""
...
def load_models(args, device):
    ...
    if args.publish:
        ...
        models = {
            ...
            "AD-EnFF plain": AdaptiveENFF(args.dim, attention=False, state_clamp=60.0),
            "MNMEF": MNMEFCorrTerms(args.dim, clamp=60.0),
        }
...
def run_seed(seed, models, args, device, pool):
    ...
    noise_np, rho_np, sigma_np = make_training_schedules(
        1, args.steps, args.difficulty, seed, 0
    )
    observation_noise = torch.tensor(noise_np[0], device=device)
    rho = torch.tensor(rho_np[0], device=device)
    lorenz_sigma = torch.tensor(sigma_np[0], device=device)
    full_truth = []
    ...
    with torch.no_grad():
        for step in range(args.steps):
            truth = lorenz63_rk4_step(
                truth, config.DT, rho=config.TRUTH_RHO, sigma=config.TRUTH_SIGMA
            )
            for name in ensembles:
                ensembles[name] = lorenz63_rk4_step(
                    ensembles[name], config.DT,
                    rho=rho[step].view(1, 1), sigma=lorenz_sigma[step].view(1, 1)
                )
            ...
            observation_std = observation_noise[step]
            ...
            if "Analytical ENFF" in models:
                ...
                ensembles["Analytical ENFF"] = enff_output[:, -1].clamp(-60.0, 60.0)
...
def print_table(results, names):
    ...
    print("\n" + "=" * 88)
    print("MATCHED LORENZ-63 EVALUATION (mean +/- std across seeds)")
    print("=" * 88)
...
def plot_attractors(truth, trajectories, out_path):
    names = list(trajectories)
    figure = plt.figure(figsize=(12, 3.2))
    truth_values = truth[0].cpu().numpy()
    display_names = {
        "Analytical ENFF": "Analytical EnFF",
        "AD-EnFF plain": "AD-EnFF",
    }

    axis = figure.add_subplot(1, len(names) + 1, 1, projection="3d")
    axis.plot(
        truth_values[:, 0], truth_values[:, 1], truth_values[:, 2],
        color="black", linestyle="--", lw=1.0,
    )
    axis.set_title("Truth", fontsize=15, pad=1)
    axis.tick_params(axis="both", labelsize=9, pad=0)
    axis.view_init(elev=24, azim=-58)

    for index, name in enumerate(names, 2):
        values = trajectories[name][0].mean(1).cpu().numpy()
        axis = figure.add_subplot(1, len(names) + 1, index, projection="3d")
        axis.plot(
            values[:, 0], values[:, 1], values[:, 2],
            color="black", linestyle="-", lw=1.0,
        )
        axis.set_title(display_names.get(name, name), fontsize=15, pad=1)
        axis.tick_params(axis="both", labelsize=9, pad=0)
        axis.view_init(elev=24, azim=-58)
    figure.tight_layout(pad=0.2, w_pad=0.1)
...
def main():
    ...
    pool = make_attractor_pool(
        lorenz63_rk4_step, pool_size, args.dim, spinup, 42, device,
    )
    ...
    plot_attractors(first_truth, first_trajectories, args.out_dir / "attractors_3d.png")
    print(f"saved {args.out_dir / 'trajectories.png'}")
    print(f"saved {args.out_dir / 'metrics.png'}")
    print(f"saved {args.out_dir / 'attractors_3d.png'}")
